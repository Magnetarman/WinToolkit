...
$Global:GuiBridgeTraceMode = $true # Set to $true to see unrecognized job output for debugging
...
try {
    ...
    if ($menuStructure) {
        ...
        # Aggiungi categoria di test per la verifica del bridging (solo in versione temp)
        $debugCat = @{ 'Name' = 'Debug & Testing'; 'Icon' = '🧪'; 'Scripts' = @(
                [pscustomobject]@{Name = 'Test-GuiBridging'; Description = 'Verifica Progress e Messaggi'; Action = 'RunFunction' }
            )
        }
        $Global:MenuStructure += $debugCat
...
function Filter-AndFormatJobOutput {
    ...
    if ($Line -match '\[WINTOOLKIT_PROGRESS_TAG\].*Percent:\s*(?<Percent>\d+)%') {
        ...
        # Log version of progress to OutputTextBox (optional periodicity could be added)
        if ($Line -match 'Activity:\s*(?<Activity>[^|]+)\| Status:\s*(?<Status>[^|]+)') {
            $activity = $matches.Activity.Trim()
            $status = $matches.Status.Trim()
            # We only log to the box if it's not a tiny increment or if it's important
            # For now, let's log major status changes or 0/100%
    ...
    if ($Line -match '\[WINTOOLKIT_RAW_HOST_OUTPUT_TAG\](?<Text>.*)') {
        ...
        if (-not [string]::IsNullOrEmpty($messageText)) {
            # Regex updated specifically to match original WinToolkit format
            $styledRawPattern = "^\[(?<Timestamp>\d{2}:\d{2}:\d{2})\]\s*(?<Icon>[✅⚠️❌💎🔄🗂️📁🖨️📄🗑️💭⸏▶️💡⏰🎉💻📊⚙️🛡️🚀📡🔑⏳📦💽🕸️🖨️🎯🔕🔥✨📜💾💽🦊🌐])\s*(?<Rest>.*)$"
            
            # Pattern per messaggi di log standard (senza icone)
            $logPattern = "^\[(?<Timestamp>\d{2}:\d{2}:\d{2})\]\s*(?<Rest>.*)$"
            ...
            if ($messageText -match $styledRawPattern) {
                ...
                # Restore original rich icon mapping
                switch ($icon) {
                ...
                # Many core functions include the emoji in the text itself.
            ...
            elseif ($messageText -match $logPattern) {
                $restOfText = $matches.Rest.Trim()
                Write-UnifiedLog -Type 'Info' -Message $restOfText -GuiColor "#B0B0B0"
            }
    ...
    # If trace mode is ON, show unrecognized lines with a prefix
    if ($Global:GuiBridgeTraceMode) {
        # Check standard ignore patterns first
        $skip = $false
        foreach ($pattern in $bannerPatterns) { if ($Line -match $pattern) { $skip = $true; break } }
        if (-not $skip) {
            Write-UnifiedLog -Type $outputType -Message "[TRACE] $Line" -GuiColor "#808080"
        }
    }
    else {
...
function Start-NextScriptJob {
    ...
    $jobScriptBlock = {
        ...
        # Definizione funzione di test per la verifica del bridging
        function Test-GuiBridging {
            Write-StyledMessage -Type 'Info' -Text "Inizio test di comunicazione GUI..."
            Start-Sleep -Milliseconds 500
            
            for ($i = 0; $i -le 100; $i += 10) {
                # Test Show-ProgressBar shim (con TAG)
                Show-ProgressBar -Activity "Test Bridging" -Status "Verifica in corso ($i%)..." -Percent $i
                
                # Test Write-Progress shim (con TAG)
                Write-Progress -Activity "Simulazione Download" -Status "Scaricamento in corso..." -PercentComplete $i
                
                if ($i -eq 20) { Write-StyledMessage -Type 'Success' -Text "✅ Messaggio di Successo ricevuto correttamente" }
                if ($i -eq 50) { Write-StyledMessage -Type 'Warning' -Text "⚠️ Messaggio di Avviso ricevuto correttamente" }
                if ($i -eq 80) { Write-StyledMessage -Type 'Error' -Text "❌ Messaggio di Errore ricevuto correttamente" }
                
                # Test Write-Host shim (con RAW TAG)
                Write-Host "[$((Get-Date).ToString('HH:mm:ss'))] 💎 Messaggio Raw Host catturato"
                
                Start-Sleep -Milliseconds 300
            }
            
            Write-StyledMessage -Type 'Success' -Text "🎉 Test di comunicazione completato!"
        }
    ...
    # Definizione funzione di test per la verifica del bridging (all'esterno del job script block ma visibile se definita qui?)
    # No, deve stare DENTRO il jobScriptBlock per essere eseguita dal job.
    
    # Re-reading jobScriptBlock to insert the test function...
    ...
    try {
        ...
        # Reset output counters and state for new job
        $Global:LastJobOutputCount = 0 
        $Global:IsInputWaiting = $false
...
function Process-JobCompletion {
    ...
    $window.Dispatcher.Invoke([Action] {
            ...
            if ($Global:ScriptJob) {
                ...
                # Process only new lines to avoid duplication with Tick_JobMonitor
                $newOutputLines = $rawOutput | Select-Object -Skip $Global:LastJobOutputCount
                ...
                # Format and output new lines
                foreach ($line in ($newOutputLines | Where-Object { $_ -isnot [hashtable] } | Out-String -Stream)) {
                    [void](Filter-AndFormatJobOutput -Line $line)
                }
                
                # Update count just in case, though job is finishing
                $Global:LastJobOutputCount = $rawOutput.Count
            }
